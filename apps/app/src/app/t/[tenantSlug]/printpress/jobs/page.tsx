...
export default async function PrintPressJobsPage() {
  const locale = await getRequestLocale();
  const t = (key: string) => translate(locale, key);

  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-card md:p-6">
      <div className="text-lg font-semibold">{t("app.printpress.jobs.title")}</div>
      <div className="mt-2 text-sm text-gray-700">{t("app.printpress.jobs.subtitle")}</div>
