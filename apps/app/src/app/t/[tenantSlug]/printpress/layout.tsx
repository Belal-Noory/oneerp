...
import { PrintPressMobileNav } from "./PrintPressMobileNav";
import { PrintPressTabs } from "./PrintPressTabs";
...
export default async function PrintPressLayout(props: { children: React.ReactNode; params: Promise<{ tenantSlug: string }> }) {
  ...
  if (tenantId) {
    ...
    status = modsRes.ok ? (modsJson?.data ?? []).find((m) => m.id === "printpress")?.status ?? null : null;
  }
    ...
    return (
      <div className="space-y-6">
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-card md:p-6">
          <div className="text-2xl font-semibold">{t("app.printpress.title")}</div>
          <div className="mt-2 text-gray-700">{t("app.printpress.subtitle")}</div>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-card md:p-6">
        <div className="text-2xl font-semibold">{t("app.printpress.title")}</div>
        <div className="mt-2 text-gray-700">{t("app.printpress.subtitle")}</div>
        <PrintPressTabs tenantSlug={tenantSlug} />
      </div>

      <div className="pb-24 lg:pb-0">{props.children}</div>
      <PrintPressMobileNav tenantSlug={tenantSlug} />
    </div>
  );
}
